...
class VoiceActivityDetector():
    ...
    def __init__(self, data):
        ...
        self.rate = 8000.0
        self.channels = 1
        self.sample_window = 0.05 # 20 ms
        self.sample_overlap = 0.025  # 10ms
        self.speech_window = 0.5  # half a second
        self.speech_energy_threshold = 0.6  # 60% of energy in voice band
        self.speech_start_band = 85
    ...
    def detect_speech_in_windows(self, detected_windows):
        for window in detected_windows:
            if window[1] == 1.0 and self.is_speech == 0:
                self.is_speech = 1
            if window[1] == 0.0 and self.is_speech == 1:
                self.is_speech = 0
    ...
    def detect_speech(self):
        ...
        if self.sample_start < self.data.size:
            ...
            sample_end = self.sample_start + sample_window
            if sample_end >= len(data): sample_end = len(data)
            data_window = data[self.sample_start:sample_end]
            ...
            detected_windows = np.append(detected_windows, [self.sample_start, speech_ratio])
            self.sample_start += sample_overlap
            self.windows = np.append(self.windows, detected_windows)
            detected_windows = detected_windows.reshape(int(len(detected_windows) / 2), 2)
            detected_windows[:, 1] = self._smooth_speech_detection(detected_windows)
        return detected_windows

    def save_output_wav(self, labels, wav_name):
        new_sound = np.array([])
        for label in labels:
            for i in range(int(label['speech_begin'] * self.rate), int(label['speech_end'] * self.rate)):
                new_sound = np.append(new_sound, self.data[i])
        new_sound = new_sound * 1.0 / max(self.data)
        wf.write(wav_name, self.rate, new_sound)
